...
@cache.cached(timeout=3600, key_prefix="pls_avgs", unless= lambda: app.config["CACHE_OFF"])
def all_pls_avgs():
...
def get_ranking(n=None, period=None):
    ...
    # all time ranking
    if not period:
        query = db.session.query(User.total_points, User)
...
class User(UserMixin, db.Model):
    ...
    @cache.memoize(timeout=60, unless= lambda: app.config["CACHE_OFF"])
    ...
    @cache.memoize(timeout=5, unless= lambda: app.config["CACHE_OFF"])
...
class Playlist(db.Model):
    ...
    @cache.memoize(timeout=600, unless= lambda: app.config["CACHE_OFF"])
    def level(self):
